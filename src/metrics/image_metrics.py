...
class MSEMetric(BaseMetric):
    ...
    def __call__(self, lensed, reconstructed, **kwargs):
        ...
        lensed = lensed.detach().to(self.device)
        reconstructed = reconstructed.detach().to(self.device)
...
class SSIMMetric(BaseMetric):
    ...
    def __call__(self, lensed, reconstructed, **kwargs):
        ...
        lensed, reconstructed = pre_process(lensed), pre_process(reconstructed)
        metric_value = [
            self.metric(2 * lensed[i]-1, 2*reconstructed[i]-1, data_range=1.0, channel_axis=-1)
            for i in range(len(lensed))
        ]
        return np.mean(metric_value)
